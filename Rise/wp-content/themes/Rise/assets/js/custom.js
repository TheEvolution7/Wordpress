...
$(document).ready(function(){
    if ($('body').hasClass('sub-page')) {
        Pace.on('done', function() {
            $('#preloader').delay(1200).fadeOut(800);
            setTimeout(() => {
                $('.banner-img  .bg-light-under').addClass('on-show')
            }, 1200);
        });
    }
    setTimeout(() => {
        ...
        owl.on('changed.owl.carousel', function(event) {
            ...
            $('bg-light-under').removeClass('on-show')
            $('.butn-light').removeClass('animated fadeInUp');
            $('.owl-item').not('.cloned').eq(item).find('bg-light-under').addClass('on-show')
